...
fn out_as_err(err: &str, out: &mut [u8]) -> *const c_char {
    let msg = if err.len() + 1 > out.len() {
        "compress_scroll_batch_bytes_standard: not enough output buffer for the error message"
    } else {
        err
    };
...
pub unsafe extern "C" fn compress_scroll_batch_bytes_standard(
    ...
    if ret.len() > buf_size as usize {
        return out_as_err(
            "compress_scroll_batch_bytes_standard: not enough output buffer for compressed data",
            out,
        );
    }
