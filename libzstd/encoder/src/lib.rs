...
pub fn init_zstd_encoder(max_block_size: u32) -> Encoder<'static, Vec<u8>> {
    ...
    encoder
        ...
        .expect("infallible");
    // set target block size to maxium block size, so each input block can always
    // be compressed into a compressed block
    encoder
        .set_parameter(CParameter::TargetCBlockSize(N_BLOCK_SIZE_TARGET))
        .expect("infallible");
    // set max block size (if specified value different to default)
    if max_block_size != N_BLOCK_SIZE_TARGET {
        encoder
        .set_parameter(CParameter::MaxBlockSize(max_block_size))
        .expect("infallible");
    }
